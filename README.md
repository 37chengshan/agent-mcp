...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/tools-tools%2Flist-3E7A56" alt="tools">
...
版本 **v4.0.0a1** · 单一来源 [`agent_mcp/__init__.py`](agent_mcp/__init__.py) · [CHANGELOG](CHANGELOG.md) · [v4 路线图](docs/plans/2026-08-28-v4-roadmap.md)
...
> 其它 CLI / DSH：见 [install-guide.md](docs/install-guide.md) · [dsh-integration.md](docs/dsh-integration.md)

### 2. 打开控制台

```bash
python3 start_agent_mcp.py --open
# → http://127.0.0.1:8765（token 走 URL fragment）
```
...
> 完整工具目录以 **`mcp_server.py` 的 `tools/list`** 与 [capability-matrix.md](docs/capability-matrix.md) 为准；README / 安装指南不再单独维护固定数量。

诚实标注：沙箱 SANDBOX_MAP 尚未接执行链（生效的是适配器 PERMISSION_FLAGS）；容器沙箱 `AGENT_MCP_SANDBOX_IMAGE` 实验开关；Prime Agent 适配器已接入、真实冒烟 ⏳。
...
flowchart TB
  ...
  MCP --> CP
  CP --> Store["store_v4 · db"]
  CP --> EM["Execution Manager · Run"]
  EM --> Ad["Backend Adapters"]
  Ad --> RT["Native CLIs"]
```

<p align="center"><img src="docs/architecture-v4.svg" width="100%" alt="v4 架构"></p>
...
更多：[architecture.md](docs/architecture.md) · [runtime.md](docs/runtime.md) · [workflow.svg](docs/workflow.svg)
